...
contract AutoFarmV2Facet is ReentrancyGuard {
    ...
    function poolInfo(
        uint256 pid
    ) external view returns (LibDiamond.PoolInfo memory) {
        LibDiamond.AutoFarmV2Storage storage a = LibDiamond.autoFarmV2Storage();
    ...
    function userInfo(
        uint256 pid,
        address user
    ) external view returns (LibDiamond.UserInfo memory) {
        LibDiamond.AutoFarmV2Storage storage a = LibDiamond.autoFarmV2Storage();
    ...
    function stakedWantTokens(
        ...
        uint256 sharesTotal = IStrategy(pool.strat).sharesTotal();
        uint256 wantLockedTotal = IStrategy(a.poolInfo[_pid].strat)
            .wantLockedTotal();
        if (sharesTotal == 0) {
            return 0;
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        AUTOToken(a.AUTOv2).mint(
            LibDiamond.contractOwner(),
            AUTOReward.mul(a.ownerAUTOReward).div(1000)
        );
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            ...
            uint256 sharesAdded = IStrategy(a.poolInfo[_pid].strat).deposit(
                _wantAmt
            );
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            uint256 sharesRemoved = IStrategy(a.poolInfo[_pid].strat).withdraw(
                _wantAmt
            );
    ...
    function set(
        ...
        a.totalAllocPoint = a
            .totalAllocPoint
            .sub(a.poolInfo[_pid].allocPoint)
            .add(_allocPoint);
