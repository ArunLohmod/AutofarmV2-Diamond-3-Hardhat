...
contract StratX2Facet is ReentrancyGuard, Pausable {
    ...
    function withdraw(
        ...
        uint256 sharesRemoved = _wantAmt.mul(s.sharesTotal).div(
            s.wantLockedTotal
        );
        ...
        if (s.withdrawFeeFactor < s.withdrawFeeFactorMax) {
            _wantAmt = _wantAmt.mul(s.withdrawFeeFactor).div(
    ...
    function _distributeFees(
        ...
        if (_earnedAmt > 0) {
            ...
            if (s.controllerFee > 0) {
                uint256 fee = _earnedAmt.mul(s.controllerFee).div(
                    s.controllerFeeMax
                );
    ...
    function _buyBack(uint256 _earnedAmt) internal virtual returns (uint256) {
        ...
        uint256 buyBackAmt = _earnedAmt.mul(s.buyBackRate).div(
            s.buyBackRateMax
        );
